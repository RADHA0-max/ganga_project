document.addEventListener('DOMContentLoaded', () => {
    ...
    const expertReportsSection = document.getElementById('expert-reports-section');
    ...
    function renderMyPosts() {
        const posts = JSON.parse(localStorage.getItem('ngoPosts')) || [];
        myPostsContainer.innerHTML = posts.length === 0 ? '<p>You have not published any posts yet.</p>' : '';
        posts.forEach(post => {
            const postEl = document.createElement('div');
            postEl.className = 'card';
            postEl.innerHTML = `<h4>${post.title}</h4><p>${post.content}</p><span class="post-date">Published on: ${post.date}</span>`;
            myPostsContainer.appendChild(postEl);
        });
    }
    function handlePostSubmission(e) {
        e.preventDefault();
        const newPost = { title: document.getElementById('post-title').value, content: document.getElementById('post-content').value, date: new Date().toLocaleDateString() };
        const posts = JSON.parse(localStorage.getItem('ngoPosts')) || [];
    ...
    function showSection(sectionId) {
        [analysisResults, userReportsSection, myProfileSection, expertReportsSection].forEach(s => s && s.classList.add('hidden'));
        const sectionToShow = document.getElementById(sectionId);
        if (sectionToShow) {
            sectionToShow.classList.remove('hidden');
            if (sectionId === 'my-profile-section') renderMyPosts();
        }
    }
    
    analysisMenu.addEventListener('click', (e) => {
        const targetLi = e.target.closest('li');
        if (targetLi && targetLi.dataset.analysis) {
            const analysisType = targetLi.dataset.analysis;
            analysisMenu.querySelector('li.active')?.classList.remove('active');
            targetLi.classList.add('active');
            if (['user-reports', 'my-profile', 'expert-reports'].includes(analysisType)) showSection(`${analysisType}-section`);
    ...
    if (initialActiveLink) {
        const initialSection = initialActiveLink.dataset.analysis;
        if (['user-reports', 'my-profile', 'expert-reports'].includes(initialSection)) showSection(`${initialSection}-section`);
        else { showSection('analysis-results'); fetchAndDisplayChartData(initialSection); }
    }
});